...
```{r, echo=FALSE}
setnames(anonymisation.keys, 'PT_ID', 'PT_ID.H1')
chains <- merge(as.data.table(dchain), anonymisation.keys, by.x = 'H1', by.y = 'AID')
setnames(anonymisation.keys, 'PT_ID.H1', 'PT_ID.H2')
chains <- merge(chains, anonymisation.keys, by.x = 'H2', by.y = 'AID')
chains <- chains[, .(PT_ID.H1, PT_ID.H2, LINK_12, LINK_21, SCORE_LINKED, SCORE_DIR_12, SCORE_DIR_21)]
```
