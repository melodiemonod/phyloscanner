...
load(path.to.age.stratification)
load(path.to.phylogeography.data)
fit <- readRDS(path.to.stan.samples)

# change formats
mc <- list()
params <- extract(fit,pars=c(paste0('llbd[',rep(1:stan_data$N_group,each=stan_data$N_per_group),',',rep(1:stan_data$N_per_group,times=stan_data$N_group),']'),
                             paste0('lxi_pair[',rep(1:stan_data$N_group,each=stan_data$N_per_group),',',rep(1:stan_data$N_per_group,times=stan_data$N_group),']')))
...
control			<- list(burnin.p=0,
                   thin=1,
                   regex_pars='*',
                   outfile=path.output.aggregated.age.source.gender)
...
control			<- list(quantiles= c('CL'=0.025,'IL'=0.25,'M'=0.5,'IU'=0.75,'CU'=0.975),
                   flowratios= list( ))
pars <- data.table(read.csv(path.output.aggregated.age.source.gender))
...
ans        <- copy(z)
gc()
write.csv(ans, row.names=FALSE,file=path.output.aggregated.age.source.gender.normgender)
...
control	<- list(burnin.p=0,
                thin=1,
                regex_pars='*',
                outfile=path.output.aggregated.age.gender)
...
ans        <- copy(z)
gc()
write.csv(ans, row.names=FALSE,file=path.output.aggregated.age.gender.normgender)
...
ans        <- copy(z)
gc()
write.csv(ans, row.names=FALSE,file=path.output.aggregated.age.gender.normgender.recage)
...
ans        <- copy(z)
gc()
write.csv(ans, row.names=FALSE,file=path.output.aggregated.age.gender.normgender.sourceage)
...
# plots
tmp <- data.table(read.csv(path.output.aggregated.age.gender.normgender))
...
# 
df <- data.table(read.csv(path.output.aggregated.age.source.gender.normgender))
...
# 
pars <- data.table(read.csv(path.output.aggregated.age.gender.normgender.sourceage))
pars[,TR_SEX:=gsub('^(.)\\:(.+)$','\\1',TR_TARGETCAT)]
pars[,TR_AGE_AT_MID_C:=gsub('^(.)\\:(.+)$','\\2',TR_TARGETCAT)]
pars[,REC_SEX:=gsub('^(.)\\:(.+)$','\\1',REC_TARGETCAT)]
pars[,REC_AGE_AT_MID_C:=gsub('^(.)\\:(.+)$','\\2',REC_TARGETCAT)]
